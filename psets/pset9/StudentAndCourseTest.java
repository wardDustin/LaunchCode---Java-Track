...
//		No credits, should be a freshman, no GPA
	...
	public void testComputeTuition() {
		...
		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly1", 20000.0, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly2", 20000.0*2, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly3", 20000.0*3, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly4", 20000.0*4, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly5", 20000.0*5, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly6", 20000.0*6, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly7", 20000.0*7, s.computeTuition());
		}

		for (int i = 0; i < 15; i++) {
			s.submitGrade(0, 1);
			assertEquals("Compute tuition not working properly8", 20000.0*8, s.computeTuition());
		}
	}
	...
	public void testGenerateRoster(){
		...
		for (int i = 0; i < 20; ++i) {
			...
			Course c2 = new Course("" + x, y, 2);
			String aa =  "" + (Math.random() * 5000);
			String bb =  "" + (Math.random() * 5000);
			int cc = (int)Math.random() * 500000;
			Student s1 = new Student(aa, bb, cc);
			String dd =  "" + (Math.random() * 5000);
			String ee =  "" + (Math.random() * 5000);
			int ff = (int)Math.random() * 500000;
			Student s2 = new Student(dd, ee, ff);
			String gg =  "" + (Math.random() * 5000);
			String hh =  "" + (Math.random() * 5000);
			int ii = (int)Math.random() * 500000;
			Student s3 = new Student(gg, hh, ii);
			c2.addStudent(s1);
			c2.addStudent(s2);
			c2.addStudent(s3);
			assertEquals("Error generating roster", "[" + aa + ' ' + bb + ' ' + cc + ", " + dd + ' ' + ee + ' ' + ff + "]", c2.generateRoster());
		}
	}
